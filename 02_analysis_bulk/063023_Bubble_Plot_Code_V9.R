#### Bubble Plot Code - with Filtered Transcript Background ####
...
# DTU, Up and Down
DTU_Clustered <- read.table("Lists_for_DAVID/Output_from_DAVID/063023_DAVID_Output/063023_DTU_Clustered.txt", sep = "\t", fill = TRUE)
DTU_Not_Clustered <- read.table("Lists_for_DAVID/Output_from_DAVID/063023_DAVID_Output/063023_DTU_Unclustered.txt", sep = "\t", fill = TRUE)
...
DTU_Clustered <- DTU_Clustered[c(3:6, 9:12, 15:18),]
dim(DTU_Clustered)
# [1] 12 13
...
# DTU, Up, Down, Up and Down
DTU_Not_Clustered <- DTU_Not_Clustered[2:9,]
...
data_DTU <- rep(c("Cluster1","Cluster2","Cluster3","Unclustered"), times = c(4,4,4,8))
...
BubblePlot <- ggplot2::ggplot(data = Combined_DTU, mapping = aes(x=Fold_Enrichment, # x axis
  ...
  scale_size_continuous(limits= c(1,400),breaks=c(100, 200, 300, 400),range = c(1,7)) + # limits sets the min and max count to be included, breaks defines the size of bubbles included on the legend, range sets the min and max size of bubbles after transformation
  ...
  coord_cartesian(xlim = c(1,1.8)) + # set min and max for x-axis
