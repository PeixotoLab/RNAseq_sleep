#### Bubble Plot Code - with DGE Background ####
...
# DTU, Up and Down
DTU_Clustered <- read.table("Lists_for_DAVID/Output_from_DAVID/041723_DAVID_Output/041723_DTU_All_Clustered.txt", sep = "\t", fill = TRUE)
DTU_Not_Clustered <- read.table("Lists_for_DAVID/Output_from_DAVID/041723_DAVID_Output/041723_DTU_All_Unclustered.txt", sep = "\t", fill = TRUE)
...
BubblePlot <- ggplot2::ggplot(data = Combined_DTU, mapping = aes(x=Fold_Enrichment, # x axis
  ...
  scale_size_continuous(limits= c(1,400),breaks=c(100, 200, 300, 400),range = c(1,10)) + # limits sets the min and max count to be included, breaks defines the size of bubbles included on the legend, range sets the min and max size of bubbles after transformation
